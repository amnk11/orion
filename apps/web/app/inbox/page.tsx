      ...
      <Card className="max-w-lg w-full shadow-lg border-slate-200/80 dark:border-slate-800">
        <CardHeader className="space-y-1">
          <div className="flex items-center justify-between">
            <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-200">
              Destination Portal (CHC / DH Desk)
            </Badge>
            {user?.role && (
          ...
          <CardTitle className="text-2xl pt-2">Destination Inbox</CardTitle>
          <CardDescription>
            Receiving Facility Referral Inbox — Patient Handoff Management
          </CardDescription>
        </CardHeader>
        ...
        <CardContent className="space-y-4">
          {session ? (
            ...
            <div className="text-center py-4 space-y-3">
              <p className="text-sm text-slate-600 dark:text-slate-400">
                Please sign in to access the destination inbox.
              </p>
              <Link href="/login">
          ...
          <div className="text-xs text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-900 p-3 rounded border border-slate-200 dark:border-slate-800">
            📌 <strong>Phase 1 Landing Verification</strong>: Redirect to destination inbox succeeded. Real-time referral triaging, accept/reject, and bed allocation workflow will be implemented in Phase 4.
          </div>
        </CardContent>
