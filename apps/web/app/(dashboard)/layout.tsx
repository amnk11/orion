...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const SidebarContent = () => (
    <>
      <div className="h-16 flex items-center justify-start px-4 border-b border-border shrink-0">
        <Link
          href="/app"
          className="flex items-center gap-2.5 w-full hover:opacity-90 transition-opacity"
        >
          <Image
            src="/sahay-small.svg"
            alt="Sahay Logo"
            width={28}
            height={28}
            className="rounded-sm object-left"
          />
      ...
      <nav className="flex-1 py-4 px-3 space-y-1 overflow-y-auto">
        ...
        {isOrigin && (
          <>
            <div className="px-3 py-2 text-xs font-medium text-muted-foreground mb-1 mt-2">
              Origin Desk
            </div>
            <NavItem href="/app" icon={Inbox} exact>
              My Referrals
            </NavItem>
            <NavItem href="/app/new/patient" icon={FilePlus2}>
              New Referral
            </NavItem>
            <NavItem href="/app/follow-ups" icon={ListChecks}>
              Follow-ups
            </NavItem>
          </>
        )}
        ...
        {isDestination && (
          <>
            ...
            <NavItem href="/destination" icon={Inbox} exact>
              Incoming Referrals
            </NavItem>
            <NavItem href="/destination/capabilities" icon={ClipboardCheck}>
              Capabilities
            </NavItem>
          </>
        )}
        ...
        {isSupervisor && (
          <>
            ...
            <NavItem href="/supervisor" icon={LayoutDashboard}>
              Analytics
            </NavItem>
          </>
        )}
      ...
      <div className="p-4 border-t border-sidebar-border bg-sidebar">
        ...
        <div className="text-xs text-muted-foreground truncate mb-4">{user.email}</div>
        <button
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-background border-t border-border pb-[env(safe-area-inset-bottom)]">
        <div className="flex items-center justify-around h-16 px-1">
          ...
          <Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
            ...
            <SheetContent
              side="bottom"
              className="p-0 rounded-t-2xl max-h-[85vh] flex flex-col gap-0 border-t border-border bg-background"
            >
  ...
  return (
    <div className="flex h-dvh bg-background overflow-hidden relative">
      ...
      >
        <OfflineBanner />
        <div id="main-content" className="flex-1 overflow-y-auto w-full max-w-[100vw]">
          {children}
        </div>
      </main>
